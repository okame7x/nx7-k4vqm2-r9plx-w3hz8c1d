...
local function randName(prefix)
	return (prefix or "") .. string.format("%x", math.random(0x10000000, 0x7fffffff))
end
...
-- Discord/http image → getcustomasset (Roblox Image can't load raw https)
local function loadHttpImage(url, cacheName)
	if typeof(url) ~= "string" or url == "" then
		return ""
	end
	if string.find(url, "rbxassetid://", 1, true) or string.find(url, "rbxasset://", 1, true) then
		return url
	end
	if typeof(getcustomasset) ~= "function" or typeof(writefile) ~= "function" then
		return ""
	end
	local path = cacheName or "ch_http_bg.jpg"
	if typeof(isfile) == "function" and isfile(path) then
		local okCached, assetCached = pcall(getcustomasset, path)
		if okCached and typeof(assetCached) == "string" and #assetCached > 0 then
			return assetCached
		end
	end
	...
	if typeof(req) == "function" then
		...
		if ok and res and (res.StatusCode == 200 or res.StatusCode == 0) and typeof(res.Body) == "string" then
			body = res.Body
		end
	end
	if not body and typeof(game.HttpGet) == "function" then
	...
	if typeof(body) ~= "string" or #body < 64 then
		return ""
	end
	pcall(writefile, path, body)
	local okAsset, asset = pcall(getcustomasset, path)
	if okAsset and typeof(asset) == "string" then
		return asset
	end
	return ""
end

-- Soft shadow without rbxassetid://5028857084 (kick fingerprint).
-- Turns an ImageLabel into a rounded dark plate.
local function applySoftShadow(img, transparency, cornerPx)
	if not img then
		return
	end
	img.Image = ""
	img.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
	img.BackgroundTransparency = transparency or 0.55
	img.BorderSizePixel = 0
	local existing = img:FindFirstChildOfClass("UICorner")
	if not existing then
		existing = Instance.new("UICorner")
		existing.Parent = img
	end
	existing.CornerRadius = UDim.new(0, cornerPx or 10)
end

local DisableAnimation = PlayerGui:FindFirstChild("TouchGui")
local T1UIColor = {
	["Border Color"] = Color3.fromRGB(255, 64, 96),
	...
	["Search Icon Highlight Color"] = Color3.fromRGB(255, 64, 96),
	...
	["Title Text Color"] = Color3.fromRGB(255, 64, 96),
	...
	["Toggle Border Color"] = Color3.fromRGB(255, 64, 96),
	["Toggle Checked Color"] = Color3.fromRGB(230, 230, 230),
	["Toggle Desc Color"] = Color3.fromRGB(185, 185, 185),
	["Button Color"] = Color3.fromRGB(255, 64, 96),
	["Label Color"] = Color3.fromRGB(38, 38, 42),
	["Dropdown Icon Color"] = Color3.fromRGB(230, 230, 230),
	["Dropdown Selected Color"] = Color3.fromRGB(255, 64, 96),
	["Dropdown Selected Check Color"] = Color3.fromRGB(200, 45, 70),
	["Textbox Highlight Color"] = Color3.fromRGB(255, 64, 96),
	["Box Highlight Color"] = Color3.fromRGB(255, 64, 96),
	["Slider Line Color"] = Color3.fromRGB(255, 64, 96),
	["Slider Highlight Color"] = Color3.fromRGB(200, 45, 70),
...
btnHide.Size = UDim2.new(0, BTN_SIZE, 0, BTN_SIZE)
...
imgHide.Size = UDim2.new(0, ICON_SIZE, 0, ICON_SIZE)
...
Library.ToggleUI = function()
	...
	-- size stays the "open" look always — no shrink when closed
...
local libCreateNoti = function(Setting)
    ...
    TextLabelNoti.Text = "<font color=\"rgb(" .. tostring(color or "255,64,96") .. ")\">" .. tostring("Ghost Pipper Hub") .. "</font> " .. tostring(getgenv().TitleNameNoti or "")
...
function Library:CreateWindow(Setting)
	...
	uistr.Thickness = 1;
	uistr.Color = Color3.fromRGB(90, 45, 55);
	...
	-- Show window as soon as it exists (library used to stay hidden until ToggleUI)
	...
	TopStroke.BackgroundColor3 = Color3.fromRGB(90, 45, 55)
	...
	local colorB = tostring(Library_Function.Getcolor(getgenv().UIColor['Title Text Color'])[3])
	local color = colorR .. ',' .. colorG .. ',' .. colorB
    TextLabelMain.Text = "<font color=\"rgb(" .. tostring(color or "255,64,96") .. ")\">" .. tostring(TitleNameMain or "Ghost Pipper Hub") .. "</font> " .. tostring(getgenv().MainDesc or "")
	...
	pageControlStroke.Color = Color3.fromRGB(90, 45, 55)
	...
	function Main_Function:AddTab(PageName)
		...
		function pageFunction:AddSection(Section_Name, Toggleable, SectionGap, SectionColor)
			...
			LineShadow.ZIndex = 0
			applySoftShadow(LineShadow, 0.7, 4)
			...
			function sectionFunction:AddToggle(idk,Setting)
				...
				check.BackgroundColor3 = Color3.fromRGB(255, 64, 96)
        ...
        function sectionFunction:AddButton(Setting, Callback)
            ...
            TextColor_1.Parent = RowBG_1
             ...
             ClickArea_1.BackgroundColor3 = Color3.fromRGB(255, 64, 96)
             ...
             UIGradient_1.Color = ColorSequence.new{
                 ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 110, 130)), 
                 ColorSequenceKeypoint.new(0.4, Color3.fromRGB(255, 64, 96)), 
                 ColorSequenceKeypoint.new(0.6, Color3.fromRGB(220, 50, 75)), 
             ...
             applySoftShadow(ImageLabel_1, 0.75, 12)
...
function sectionFunction:AddKeyBind(Setting, Callback)
    ...
    return keybindFunction
        ...
        local pagefunc = {}
        function pagefunc:AddSection(name, toggleable, gap, color)
            return pageFunction:AddSection(name, toggleable, gap, color)
        end
...
local okWin, winOrErr = pcall(function()
	...
	local FarmStatus = TabFarm:AddSection("Status")
	FarmStatus:AddLabel("Auto Farm desligado.")
	local FarmControls = TabFarm:AddSection("Controls")
	FarmControls:AddToggle("AutoFarm", {
		Title = "Auto Farm",
		Desc = "Liga o loop de farm (Chicken + ForestStrike + sync)",
		Default = false,
		Callback = function() end,
	})
	FarmControls:AddDropdown("FarmMode", {
		Title = "Farmar por",
		Values = { "Gen", "Rarity" },
		Default = "Gen",
		Callback = function() end,
	})
	FarmControls:AddSlider({
		Title = "Farm Speed",
		Min = 1,
		Max = 100,
		Default = 50,
	...
	-- Plot
	--------------------------------------------------------------------
	local TabPlot = Window:AddTab("Plot")
	local PlotStatus = TabPlot:AddSection("Status")
	PlotStatus:AddLabel("Plot idle.")
	local PlotPets = TabPlot:AddSection("Pets & Eggs")
	PlotPets:AddToggle("AutoSellPet", {
		Title = "Auto Sell Pet",
		Desc = "Vende pets com gen <= 50k",
		Default = false,
		Callback = function() end,
	})
	PlotPets:AddToggle("AutoPlantEgg", {
		Title = "Auto Plant Egg",
		Desc = "Equipa egg, vai ao centro e planta",
		Default = false,
		Callback = function() end,
	})
	PlotPets:AddToggle("PlantBestGen", {
		Title = "Best Gen",
		Desc = "Prioriza maior gen no plant",
		Default = false,
		Callback = function() end,
	})
	PlotPets:AddToggle("PlantBestRarity", {
		Title = "Best Rarity",
		Desc = "Prioriza maior rarity no plant",
		Default = false,
		Callback = function() end,
	})
	PlotPets:AddToggle("AutoClaimEgg", {
		Title = "Auto Claim Egg",
		Desc = "Abre eggs prontos da plot",
		Default = false,
		Callback = function() end,
	})
	local PlotUp = TabPlot:AddSection("Upgrades")
	PlotUp:AddToggle("AutoImproveBelt", {
		Title = "Auto Improve Belt",
		Desc = "Upgrade treadmill",
		Default = false,
		Callback = function() end,
	})
	PlotUp:AddToggle("AutoImprovePlot", {
		Title = "Auto Improve Plot",
		Desc = "Upgrade base",
		Default = false,
		Callback = function() end,
	})
	...
	local ScanSec = TabMain:AddSection("Scanner")
	ScanSec:AddLabel("Scanner iniciando...")
	ScanSec:AddButton({
		Title = "Refresh Eggs",
		Callback = function()
			Library:Notify({ Content = "refresh (preview)", Duration = 2 }, true)
		end,
	})
	ScanSec:AddInput("Webhook", {
		Title = "Webhook / note",
	...
	local EspSec = TabESP:AddSection("Overlays")
	EspSec:AddToggle("EspBestRarity", {
		Title = "ESP Best Rarity",
		Desc = "Highlight + GUI no egg de maior rarity",
		Default = false,
		Callback = function() end,
	})
	EspSec:AddToggle("EspBestGen", {
		Title = "ESP Best Gen",
		Desc = "Highlight + GUI no egg de maior gen",
		Default = false,
		Callback = function() end,
	})
	EspSec:AddLabel("ESP desligado.")
	EspSec:AddDropdown("EspStyle", {
		Title = "ESP Style",
		Values = { "Box", "Highlight", "Billboard" },
		Default = "Highlight",
		Callback = function() end,
	})
	...
	local MiscSafe = TabMisc:AddSection("Safety")
	MiscSafe:AddToggle("AntiTrap", {
		Title = "Anti Trap",
		Desc = "Pausa farm quando preso em trap/debris",
		Default = false,
		Callback = function() end,
	})
	MiscSafe:AddKeyBind({
	...
	local MiscUi = TabMisc:AddSection("UI")
	MiscUi:AddButton({
		Title = "Test Notify",
		Callback = function()
			Library:Notify({
	...
	MiscUi:AddDropdown("ThemePreview", {
		Title = "Accent preview",
		Values = { "Red", "Pink", "Crimson" },
		Default = "Red",
		Search = true,
		Callback = function() end,
	})
	MiscUi:AddLabel("Left Ctrl / botão flutuante — show / hide")

	--------------------------------------------------------------------
	-- Showcase (todos os tipos)
	--------------------------------------------------------------------
	local TabShow = Window:AddTab("Showcase")
	local ShowAll = TabShow:AddSection("All controls")
	ShowAll:AddLabel("Label — texto informativo")
	ShowAll:AddToggle("demoToggle", {
		Title = "Toggle",
		Desc = "descrição do toggle",
		Default = true,
		Callback = function() end,
	})
	ShowAll:AddButton({
		Title = "Button",
		Callback = function()
			Library:Notify({ Content = "button click", Duration = 2 }, true)
		end,
	})
	ShowAll:AddDropdown("demoDrop", {
		Title = "Dropdown",
		Values = { "Option A", "Option B", "Option C", "Option D" },
		Default = "Option A",
		Callback = function() end,
	})
	ShowAll:AddDropdown("demoDropMulti", {
	...
	Library:Notify({
		Title = "preview",
		Content = "todas as opções carregadas",
		Duration = 3,
	}, true)
